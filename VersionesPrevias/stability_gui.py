...
def compute_hydrostatic_stress(x_union, L_total, H_w):
    gamma_w = 9810
    ...
    if run_analysis:
        with st.spinner("Processing data..."):
            try:
                ...
                df_hydro = compute_hydrostatic_stress(x_union, L_total, H_w)
                
                # Rotate both stress states
                df_fem = rotate_to_normal_tangential(df, theta_deg)
                df_hyd = rotate_to_normal_tangential(df_hydro, theta_deg)
                
                # Add stresses
                df = df_fem.copy()
                df["sigma_n"] += df_hyd["sigma_n"]
                df["tau"] += df_hyd["tau"]
