...
def plot_dam_profile(geom: Geometry, R_vec, e_m, title):
    fig, ax = plt.subplots(figsize=(10, 8))
    poly = geom.poly.copy()
    closed = np.vstack([poly, poly[0]])
    ax.plot(closed[:,0], closed[:,1], 'b-', linewidth=2, label="Perfil")
    
    ax.plot([0, geom.toe_ad[0]], [0, geom.toe_ad[1]], 'k--', linewidth=1)
...
def plot_base_pressures(geom: Geometry, N_kN, e_m, title):
    B = geom.B_len
    if B <= 0:
        return None

    sigma_med = N_kN / B
    sigma_AU = sigma_med * (1.0 + 6.0 * e_m / B) if np.isfinite(e_m) else 0.0
    sigma_AD = sigma_med * (1.0 - 6.0 * e_m / B) if np.isfinite(e_m) else 0.0

    s = np.array([0.0, B])
    sig = np.array([sigma_AU, sigma_AD])

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(s, sig, linewidth=2, marker='o')
    ax.axhline(0, linestyle="--", color='red', alpha=0.5)
    ax.fill_between(s, 0, sig, alpha=0.3)
    ax.set_xlabel("s a lo largo de base (m)  [0=toe AU → B=toe AD]")
    ax.set_ylabel("σ normal (kPa)")
    ax.set_title(title)
...
def main():
    ...
    with st.sidebar:
        ...
        cohesion_kPa = st.number_input("Cohesión (kPa)", 
                                       value=st.session_state.config["MATERIALES"]["cohesion_kPa"], 
                                       min_value=0.0, format="%.2f")
        
        st.markdown("---")
        if st.button("🔄 Actualizar Geometría", type="primary", use_container_width=True):
            try:
        ...
        with col1:
            ...
            if st.button("Agregar"):
                if new_esc_id and new_esc_name and new_esc_id not in st.session_state.config["ESCENARIOS"]:
                    ...
                    st.session_state.config["FACTORES_MINIMOS_FS"][new_esc_id] = {"desliz": 1.0, "vuelco": 1.0}
    ...
    for idx, esc_id in enumerate(escenarios_list):
        with tabs[idx + 1]:
            ...
            with col1:
                ...
                H_AU = st.number_input("H AU (m)", value=esc_data.get("H_AU", 0.0), 
                                       min_value=0.0, format="%.2f", key=f"{esc_id}_H_AU")
                H_AD = st.number_input("H AD (m)", value=esc_data.get("H_AD", 0.0), 
                                       min_value=0.0, format="%.2f", key=f"{esc_id}_H_AD")
                H_AU_subp = st.number_input("H AU subpresión (m)", value=esc_data.get("H_AU_subpresion", 0.0), 
                                            min_value=0.0, format="%.2f", key=f"{esc_id}_H_AU_subp")
                H_AD_subp = st.number_input("H AD subpresión (m)", value=esc_data.get("H_AD_subpresion", 0.0), 
            ...
            with col3:
                ...
                usar_cohesion = st.selectbox("Usar cohesión", 
                                            options=["with_c", "without_c"], 
                                            index=0 if regla_c == "with_c" else 1,
                                            key=f"{esc_id}_cohesion")
            ...
            if esc_id in st.session_state.results:
                ...
                with col_g1:
                    st.markdown("**Perfil y Resultante**")
                    try:
                        fig_perfil = plot_dam_profile(
                            st.session_state.geometry,
                            result['R'],
                            result['base']['e_m'],
                            f"{esc_id} - {nombre}"
